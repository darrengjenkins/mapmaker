...
/** Already drawn as US atlas subdivisions. */
const US_ATLAS_CARIBBEAN = new Set([
  "Puerto Rico",
  "United States Virgin Islands",
]);

const picks = j.features.filter((f) => {
  if (f.properties.SUBREGION !== "Caribbean") return false;
  return !US_ATLAS_CARIBBEAN.has(f.properties.ADMIN);
});
...
const geoPath = path.join(root, "public/geo/caribbean-countries.json");
const tsPath = path.join(root, "src/lib/caribbeanCountryIso.ts");
